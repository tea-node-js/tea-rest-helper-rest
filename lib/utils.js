...
const findOptFilter = (params, name, where, col = name) => {
  ...
  if (_.isString(params[`${name}s!`])) {
    if (!where[col]) where[col] = {};
    where[col][Op.not] = params[`${name}s!`].trim().split(',');
  }
...
const findAllOpts = (Model, params, isAll) => {
  ...
  if (includes) {
    _.each(includes, (x) => {
      const includeWhere = {};
      const filterAttrs = _.keys(x.model.rawAttributes);
      ...
      if (x.model.rawAttributes.isDelete && !params.showDelete) {
        includeWhere[Op.or] = [{ isDelete: 'no' }];
      ...
      if (Object.getOwnPropertySymbols(includeWhere).length) x.where = includeWhere;
  ...
  if (_searchOrs.length) where[Op.or] = [[mergeSearchOrs(_searchOrs), ['']]];
 ...
 */
const callback = (promise, cb) => promise
  .then((result) => {
    cb.call(null, null, result);
  })
  .catch(cb);

const pickParams = (ctx, cols, Model) => {
  const attr = {};
  const { params, isAdmin } = ctx;
