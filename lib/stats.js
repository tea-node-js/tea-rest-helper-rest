...
const statsCount = async (Model, opts, dims) => {
  ...
  const { count = 0 } = await Model.findOne(option);
  return count;
};
...
module.exports = (rest) => {
  const statistics = async (Model, params, where, conf) => {
    const option = {};
    const dims = getDimensions(Model, params, conf && conf.dimensions);
    ...
    const count = await statsCount(Model, opt, dims);

    const results = await Model.findAll(opt);
